...
class TestLogParser(unittest.TestCase):
    ...
    def test_parse_valid_log(self):
        ...
        expected_data = {
            "ip": "192.168.1.1",
            "datetime": datetime(2024, 12, 6, 13, 0, 0, tzinfo=pytz.timezone("Europe/Moscow")),
            "status": "200",
            "response_time": 0.320,
        }
        ...
        self.assertEqual(result["response_time"], expected_data["response_time"])

        # Игнорируем временную зону при сравнении времени
        self.assertEqual(result["datetime"].replace(tzinfo=None), expected_data["datetime"].replace(tzinfo=None))
    ...
    def test_convert_to_timezone(self):
        ...
        expected_datetime = datetime(2024, 12, 6, 13, 0, 0, tzinfo=pytz.timezone("Europe/Moscow"))
        ...
        self.assertEqual(result.replace(tzinfo=None), expected_datetime.replace(tzinfo=None))
